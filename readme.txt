Name: Josiah Varghese
StudentID: 1043468
Date: October 22, 2022
CIS*2520: Assignment 2

1	Stacks
Program name: q1.c
To run: 
	>make q1
	>./q1
Sample Output:
//Output for transaction code 2, 6, and 7. 

Please select one of the following options: 
1: Add a new car to the available-for-rent list.
2: Add a returned car to the available-for-rent list.
3: Add a returned car to the repair list.
4: Transfer a car from the repair list to the available-for-rent list.
5: Rent the first available car.
6: Print all the lists.
7: Quit.

Enter transaction code: 2
Enter plate number: h19sa1                                    
Enter mileage: 230000
Charge: $4550.00
Returning Car to Available List from Rented List

Added Car to Available List

Please select one of the following options:
1: Add a new car to the available-for-rent list.
2: Add a returned car to the available-for-rent list.
3: Add a returned car to the repair list.
4: Transfer a car from the repair list to the available-for-rent list.
5: Rent the first available car.
6: Print all the lists.
7: Quit.

Enter transaction code: 6
Available cars
*-------------*
Plate: ksad941
Mileage: 51000
Return date: 0

Plate: jsao916
Mileage: 81000
Return date: 0

Plate: adns790
Mileage: 90000
Return date: 0

Plate: cpxy312
Mileage: 100000
Return date: 0

Plate: hadj841
Mileage: 102000
Return date: 0

Plate: h19sa1
Mileage: 230000
Return date: 221220

Rented cars
*-------------*
Plate: tyuq782
Mileage: 29000
Return date: 221025

Plate: 5ewz1i2
Mileage: 210001
Return date: 221115

Plate: bksa212
Mileage: 181202
Return date: 221116

Plate: 12h89s
Mileage: 120100
Return date: 221215

Cars in repair
*-------------*
Plate: ansd123
Mileage: 10000
Return date: 0

Plate: asj21a
Mileage: 91021
Return date: 0

Plate: ugyeh
Mileage: 90120
Return date: 0

Plate: asda1j1
Mileage: 30000
Return date: 0

Plate: ksla211
Mileage: 89213
Return date: 0

Please select one of the following options:
1: Add a new car to the available-for-rent list.
2: Add a returned car to the available-for-rent list.
3: Add a returned car to the repair list.
4: Transfer a car from the repair list to the available-for-rent list.
5: Rent the first available car.
6: Print all the lists.
7: Quit.

Enter transaction code: 7
Total income: $4550.00

2	Reverse Polish notation (RPN): Łukasiewicz notation
Program name: q2.c
To run: 
	>make q2
	>./q2 <expression>
Sample Output:
	.\q2 12+53-*
	6.00